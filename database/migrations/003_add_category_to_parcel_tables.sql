...
ALTER TABLE parcel_in ADD COLUMN IF NOT EXISTS category VARCHAR(50) DEFAULT 'Others';

-- Add category column to parcel_out table  
ALTER TABLE parcel_out ADD COLUMN IF NOT EXISTS category VARCHAR(50) DEFAULT 'Others';
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'check_parcel_in_category') THEN
        ALTER TABLE parcel_in ADD CONSTRAINT check_parcel_in_category 
        CHECK (category IN ('Component', 'Product', 'Tool', 'Others'));
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'check_parcel_out_category') THEN
        ALTER TABLE parcel_out ADD CONSTRAINT check_parcel_out_category 
        CHECK (category IN ('Component', 'Product', 'Tool', 'Others'));
    END IF;
END $$;
